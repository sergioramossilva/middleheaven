			...
			if(folder.exists()){
				...
				if (kinds.contains(StandardWatchEvent.ENTRY_CREATED)){
					for (ManagedFile file : newFiles){
						...
						if (file.isModificationTraceable()) {
							this.lastTimeChecked = file.getModificationTrace().getLastModified().getMilliseconds();
						}	
					}
				}
				...
				if (kinds.contains(StandardWatchEvent.ENTRY_MODIFIED)){
					for (ManagedFile file : common){
						...
						if ((file.isModificationTraceable())) {
							long lastModified = file.getModificationTrace().getLastModified().getMilliseconds();
							if (lastModified  > this.lastTimeChecked){
								events.add(new SimpleWatchEvent(file,StandardWatchEvent.ENTRY_MODIFIED));
								
								this.lastTimeChecked = lastModified;
							}
