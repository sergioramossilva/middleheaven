...
import org.middleheaven.util.EmptyIncrementor;
 ...
 */
public class Range<T, I> extends AbstractEnumerable<T> {
	...
	public boolean isEmpty(){
		return (start == end || comparator.compare(start, end) == 0) && (this.excludeEnd || this.excludeStart);
	}
     ...
     */
	public T random() {
		return this.size() == 0 ? null : index(( int )( Math.random() * this.size()));
	}
	...
	public T random(Random random) {
		return this.size() == 0 ? null : index(random.nextInt(this.size()));
	}
