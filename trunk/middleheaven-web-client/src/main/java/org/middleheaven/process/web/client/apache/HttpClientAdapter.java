...
class HttpClientAdapter implements HttpClient {
	...
	public HttpClientAdapter (DefaultHttpClient apacheClient){
		...
		this.userAgent = new HttpUserAgent(
				BrowserInfo.browser( "MiddleHeaven Client", "ApacheHttpClient", Version.valueOf(1, 0)) , 
				OperatingSystemInfo.local()
		);
	}
	...
	private HttpClientResponse processGet(HttpClientRequest request) throws HttpProcessException {
		...
		if (!qparams.isEmpty()){
			builder.append("?")
			.append(URLEncodedUtils.format(qparams, "UTF-8"));
	...
	private String createUserAgentHeaderValue () {
		return new StringBuilder(userAgent.getBrowserInfo().getBaseEngine())
		.append("/4.0 (compatible;")
		.append(userAgent.getBrowserInfo().getName()).append(" ")
		.append(userAgent.getBrowserInfo().getVersion().getMajor()).append(".")
		.append(userAgent.getBrowserInfo().getVersion().getMinor())
		.append("; ")
		.append(userAgent.getOperatingSystemInfo().getOperatingSystem()).append(" ")
		.append(userAgent.getOperatingSystemInfo().getOperatingSystemVersion())
		.append(")")
		.toString();
