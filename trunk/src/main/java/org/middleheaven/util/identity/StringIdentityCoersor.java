 ...
 */
public class StringIdentityCoersor extends AbstractIdentityCoersor<String> {
	...
	public <T extends Identity> T coerceForward(String value, Class<T> type) {
		...
		for (Class<? extends Identity> t : types){
			if (t.isAssignableFrom(type)){
				Method valueOfMethod = Introspector.of(t).inspect().staticMethods()
					.named("valueOf")
				...
				return Introspector.of(valueOfMethod).invoke(type, null, value);
			}
		}
	...
	private <T extends Identity> T fromString(String value, Class<T>  type){
		...
		for (Class<? extends Identity> t : types){
			if (parts[0].equals(t.getSimpleName().substring(0,1))){
				...
				return Introspector.of(valueOfMethod).invoke(type, null, parts[1]);
			}
		}
