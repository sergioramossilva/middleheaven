...
public class GaussJordanInvertion {
	...
	public static <F extends Field<F>> Matrix<F> invert( Matrix<F> matrix){
		...
		EditableMatrix<F> A = EditableMatrix.augmentWithEntity(matrix);
		...
		EditableMatrix<F> I =  new EditableMatrix<F>(A,0 , A.columnsCount()/2,3,3);
	...
	// post: A[i][j]==1;
	private static <F extends Field<F>>  void divide(EditableMatrix<F> A, int i, int j,F ONE){
	...
	private static <F extends Field<F>>  void eliminate(EditableMatrix<F> A, int i, int j, F ZERO){
