...
//		assertEquals(I, P.times(P.inverse()));
	...
	public void testMeasures(){
		Measure L = Measure.measure(3, 0.2 , SI.METER);
		Measure F = Measure.measure(5, 0.1 , SI.METER);
		
		Measure S = F.plus(L);
		assertEquals (Measure.measure(8, 0.3, SI.METER) , S);
		
		Measure D = F.times(L);
		assertEquals (Measure.measure(15, 1.30, SI.METER.raise(2)) , D);
		...
		Measure l = Measure.exact(200, SI.METER );
		Measure t = Measure.exact(10, SI.SECOND);
		Measure v = l.over(t);
		
		assertEquals(Measure.exact(20,  SI.METER.over(SI.SECOND) ), v);
		assertEquals(Dimension.VELOCITY, v.unit().dimension());
		
		Measure v2 = v.times(v);
		Dimension dim =  Dimension.VELOCITY.times(Dimension.VELOCITY);
		assertEquals(dim,v2.unit().dimension());
		Measure m = Measure.exact(50, SI.KILOGRAM );
		Measure y = m.times(v2);
		Measure EC = y.times(Real.valueOf(0.5));
