...
public class ArrayManagedFilePath implements ManagedFilePath , Watchable {
	...
	public ManagedFilePath resolve(String name) {
		return new ArrayManagedFilePath(this.repository, this.root,  CollectionUtils.addToArray(this.names, Splitter.on("/").split(name).asArray()));
	}
	...
	public ManagedFilePath resolveSibling(String name) {
		String[] pathNames = Splitter.on("/").split(name).asArray();
