...
public class HttpProcessingUtils {
	...
	public static HttpUserAgent parse(HttpServletRequest request) {
		...
		List<Culture> acceptableLanguages = acceptableLanguages(request.getHeader("Accept-Language"));

		BrowserRawInfo res = getBrowser(userAgent);
		BrowserInfo binfo;

		if (res==null){
			binfo = BrowserInfo.unkownBrowser(acceptableLanguages);
			binfo.name = userAgent;
		} else {
			binfo = BrowserInfo.browser(acceptableLanguages, res.name, res.engine,VersionReader.fromString(res.version));
			binfo.bot = res.isBot;
		}
	 ...
	 static List<Culture> acceptableLanguages(String acceptableLanguage) {

		if (acceptableLanguage != null && !acceptableLanguage.trim().isEmpty()) {
			// see http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html
			
			// example : da, en-gb;q=0.8, en;q=0.7

			if (acceptableLanguage.contains(",")){
				String[] langs = acceptableLanguage.split(",");
				...
				cultures.add(resolveCulture(acceptableLanguage.trim()));
			}
		}
