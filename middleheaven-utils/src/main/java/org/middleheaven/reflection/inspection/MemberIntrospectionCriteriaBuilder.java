...
public abstract class MemberIntrospectionCriteriaBuilder<T, M> {
	...
	protected void addWithStaticFilter(final boolean allowStatic){
		logicClassifier.add(new Predicate<M>(){
			...
			@Override
			public Boolean apply(M member) {
				return !(allowStatic ^ Modifier.isStatic(getModifiers(member)));
			}
		});
	}
	...
	protected abstract Enumerable<M> getAllMembersInType(ReflectedClass<T> type);
}
